            ...
            <div class="col-md-8">
                <article class="post-single">
                    <div class="post-info">
                        ...
                        <h6 class="upper"><span>By</span><a href="#"> {{ $single_post->user->name }}</a><span class="dot"></span><span>{{ date('d F, Y', strtotime($single_post->created_at)) }}</span><span class="dot"></span> @foreach($single_post->tags as $tag) <a href="{{ route('post.tag.search', $tag -> slug) }}" class="post-tag">{{ $tag->name }}<span class="sptcoma">,</span></a> @endforeach </h6>
                    </div>
